...
def imshow_det_bboxes(img,
    ...
    # add for nus gt-aug
    os.makedirs(f"nuscenes_gt_database_img/img", exist_ok=True)
    os.makedirs(f"nuscenes_gt_database_img/txt", exist_ok=True)
    for i, (bbox, label) in enumerate(zip(bboxes, labels)):
        ...
        cam_name = out_file_s[['CAM' in a for a in out_file_s].index(True)]
        img_name = f"{token}_{class_names[label]}_{i}_{cam_name}"

        if mask.nonzero()[0].shape[0] == 0:
            continue
        x_min, x_max = mask.max(1).nonzero()[0][0], mask.max(
            1).nonzero()[0][-1]
        y_min, y_max = mask.max(0).nonzero()[0][0], mask.max(
            0).nonzero()[0][-1]
        crop_img = cv2.copyTo(img_copy, mask.astype(np.uint8))[x_min:x_max,
                                                               y_min:y_max]
        if x_max - x_min == 0 or y_max - y_min == 0:
            continue
        if crop_img.max(2).nonzero()[0].shape[0] / (crop_img.shape[0] *
                                                    crop_img.shape[1]) < 0.7:
            continue
        if x_max - x_min < 30 or y_max - y_min < 30:
            continue

        mmcv.imwrite(crop_img, f"nuscenes_gt_database_img/img/{img_name}.png")
        with open(f"nuscenes_gt_database_img/txt/{img_name}.txt", "w") as f:
            f.write('%d %d %d %d' % (x_min, x_max, y_min, y_max))
